  ...
  return (
    <div>
      <ButtonGroup>
        ...
        <Button disabled={quantity < 2} onClick={() => setQuantity(prev => prev - 1)}>-</Button>
        <Button style={{ color: "black" }} disabled><strong>{quantity}</strong></Button>
        <Button onClick={() => setQuantity(prev => prev + 1)}>+</Button>
        </Fragment>
      :
        <Button variant="text" sx={{ color: "black" }}>Quantity: {quantity}</Button>
      }
      </ButtonGroup>
      {!checkout && !loading && <Button fullWidth onClick={onSubmit} variant='outlined'>Add to Order</Button>}
      {loading &&
        <LoadingButton fullWidth loading variant="outlined">
          Submit
        </LoadingButton>}
      {checkout && <Button fullWidth color="success">Added!</Button>}
      <Button fullWidth component={Link} to={`/orders/cart`}>View Cart</Button>
    </div>
  )
